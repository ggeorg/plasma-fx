...
import plasma.fx.scene.webview.MyRendererContext;
import plasma.fx.scene.webview.UserAgentContext;
import plasma.fx.scene.webview.js.AbstractScriptableDelegate;
import plasma.fx.scene.webview.style.RenderState;
import plasma.fx.scene.webview.style.StyleSheetRenderState;
import plasma.fx.scene.webview.util.Strings;
...
public abstract class NodeImpl extends AbstractScriptableDelegate implements Node, ModelNode {
  ...
  private ChildMyElementCollection childrenCollection;

  public ChildMyElementCollection getChildren() {
    // Method required by JavaScript
    synchronized (this) {
      ChildMyElementCollection collection = this.childrenCollection;
      if (collection == null) {
        collection = new ChildMyElementCollection(this);
  ...
  public UserAgentContext getUserAgentContext() {
    ...
    if (doc instanceof MyDocumentImpl) {
      return ((MyDocumentImpl) doc).getUserAgentContext();
    }
    else {
      return null;
    }
  ...
  public MyRendererContext getHtmlRendererContext() {
    Object doc = this.document;
    if (doc instanceof MyDocumentImpl) {
      return ((MyDocumentImpl) doc).getHtmlRendererContext();
    }
    else {
      return null;
    }
  ...
  public URL getFullURL(String spec) throws MalformedURLException {
    Object doc = this.document;
    if (doc instanceof MyDocumentImpl) {
      return ((MyDocumentImpl) doc).getFullURL(spec);
    }
    else {
  ...
  public URL getDocumentURL() {
    Object doc = this.document;
    if (doc instanceof MyDocumentImpl) {
      return ((MyDocumentImpl) doc).getDocumentURL();
    }
    else {
      return null;
    }
  ...
  public final boolean isEqualOrDescendentOf(ModelNode otherContext) {
    ...
    if (parent instanceof MyElementImpl) {
      return ((MyElementImpl) parent).isEqualOrDescendentOf(otherContext);
    }
    else {
      return false;
    }
    ...
    if (nl != null && (size = nl.size()) > 0) {
      for (int i = 0; i < size; i++) {
        ...
        if (child instanceof MyElementImpl) {
          ((MyElementImpl) child).appendOuterHTMLImpl(buffer);
        }
